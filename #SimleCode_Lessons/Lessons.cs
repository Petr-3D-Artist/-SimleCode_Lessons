        ...
        {                                          // Lesson 4 and 5  // Типы данных // ПЕРЕМЕННЫЕ // ИНИЦИАЛИЗАЦИЯ
           ...
           */
            ...
            */
            ...
            //Массив - это совокупность элементов одного типа, некая коллекция данных
            //В строго типизированных языках такик как С# любые данные которые являются элементами массива должны быть одного и тогоже тип
            //Каждый элемент они обьеденены одним именем и доступ ко всем элементам мы сможем получать через одного и тоже имя
            //Имя и индекс - пример - myArray (имя) - [Тут наш идекс]

            //Ecample:
            //[] - это для обьявления массива
            /*
            int[] myArray; // обьявление массива - назвали myArray
            myArray = new int[5]; //new int[5]; - выделение операт. память мод массив // [5] - кол-во элементов в нашем массиве 
            myArray[1] = 3;
            myArray[0] = 10;
            int a = myArray[0]; // Мы извлекаем под индексом [0] наш массив и помещаем в нашу переменную а
            Console.WriteLine(a);
            Console.ReadLine(); 
            */


            //LESSON 32.ИНИЦИАЛИЗАЦИЯ МАССИВА - СПОСОБЫ
            /*
            // int[] myArray = new int[] { 1, 2, 3 }; //сюда [] можно не вписывать кол-во элементов, но если не будет {1,2,3}; скобок - то не сработает
            // int[] myArray = { 1, 2, 3 }; // Можно сделать и так
            int[] myArray = Enumerable.Repeat(5, 10).ToArray(); // Repeat - он повторяет, какой-то элемент энное кол-во раз. Создает коллекцию. 
                                              // Эта конструкция создаст массив из 10 элементов и каждый этот элемент будет содержать в себе 5
            ...
            */
            ...
            for (int i = myArray.Length - 1; i >= 0; i--) //тут и есть принцип вывода обратного порядка
            {
                Console.WriteLine(myArray[i]);
            }
            Console.ReadLine();
